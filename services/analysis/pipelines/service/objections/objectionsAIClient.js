// =====================================================
// SISTEMA IRIS - Objections AI Client (AISLADO)
// Cliente IA exclusivo para el servicio OBJECTIONS
...
/**
 * Realiza la llamada IA para generacion de objeciones.
 *
 ...
 *
 * @param {Object} payload - Resultado de objectionsPayloadBuilder.build()
...
async function call(payload) {
  ...
  try {
    if (!config.promptId) {
      throw new Error('Prompt ID no configurado para el servicio OBJECTIONS');
    }
    ...
    if (inputImages && inputImages.length > 0) {
      ...
      for (const img of inputImages) {
        try {
          ...
          console.log(`[objectionsAIClient] Imagen ${img.marker} subida como archivo: ${fileId} (${img.mimeType})`);
        } catch (imgUploadErr) {
          console.warn(`[objectionsAIClient] Error subiendo imagen ${img.marker}: ${imgUploadErr.message}`);
        }
      }
    ...
    console.error(`[objectionsAIClient] ${error.message}`);
    return {
      success: false,
      content: '',
      provider: 'openai',
